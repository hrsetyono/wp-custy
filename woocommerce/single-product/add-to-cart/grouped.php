...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	<ul class="woocommerce-grouped-product-list group_table">
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				...
				echo '<li id="product-' . esc_attr( $grouped_product_child->get_id() ) . '" class="woocommerce-grouped-product-list-item ' . esc_attr( implode( ' ', wc_get_product_class( '', $grouped_product_child->get_id() ) ) ) . '">';
						...
						case 'quantity':
							...
							if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
								...
								woocommerce_quantity_input( array(
									...
									'input_value' => isset( $_POST['quantity'][ $grouped_product_child->get_id() ] ) ? wc_stock_amount( wc_clean( sanitize_text_field( wp_unslash($_POST['quantity'][ $grouped_product_child->get_id() ]) ) ) ) : 0, // WPCS: CSRF ok, input var okay, sanitization ok.
